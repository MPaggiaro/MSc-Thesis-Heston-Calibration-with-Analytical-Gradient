...
int main() {
    ...
    int n = 1; // # of observations (consistent with the struct mktpara)
...
//    }
    ...
    double kappa = 3.0;               // kappa                           |  mean reversion rate
    double theta = 0.10;              // v_infinity                      |  long term variance
    double sigma = 0.25;              // sigma                           |  variance of volatility
    double rho = -0.8;            // rho                             |  correlation between spot and volatility
    double v0 = 0.08;             // v0                              |  initial variance

    double pstar[5];
    pstar[0] = kappa; pstar[1] = theta; pstar[2] = sigma; pstar[3] = rho; pstar[4] = v0;

    double start_s = clock();
    // compute the market observations with pstar
    double x[1];
